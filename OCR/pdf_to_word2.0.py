...
# PDF处理函数：从PDF文件中提取图片
def extract_and_process_images_from_pdf(pdf_path):
    ...
    for page_num in range(len(pdf_document)):
        ...
        for img_index, img in enumerate(image_list):
            ...
            image_full = Image.open(image_stream)
            image_full_np = np.array(image_full)

            # OCR处理
            det_result = text_detection(image_full_np, ocr_detection)
            ocr_result = text_recognition(det_result, image_full_np, ocr_recognition)
            print(f"Page {page_num + 1}, Image {img_index + 1} OCR识别结果:")
            print(ocr_result)
...
if __name__ == "__main__":
    ...
    extract_and_process_images_from_pdf(pdf_path)