...
if __name__ == "__main__":
    ...
    # 加载已存在的 Excel 文件
    wb = openpyxl.load_workbook("安全物理环境数据6.0.xlsx")
    ws = wb.active
...
#     realy_result_col = existing_columns.index("realy_result") + 1
    ...
    for i, data in enumerate(security_control_class_data, start=0):
        ...
        expectResult = getExpect("安全物理环境", security_control_class_data[i], quota, 5)
        ...
        if llm == 'qwen':
            ...
            if qwen_result != result_score_data[i]:
                operationStep = getExpect("安全物理环境", security_control_class_data[i], quota, 4)
