        ...
        messages = [
            {
                ...
                最后判断，进行说明，如果每条预期结果都符合则输出"结果：符合"，每条预期结果都不符合则输出"结果：不符合"，部分预期结果符合则输出"结果：部分符合"。
                """,
            },
            {
                "role": "user",
...
if __name__ == "__main__":
    ...
    file_name = 'Ⅰ区接入服务器2.2.xlsx'
    
    instructionFile = '04卓识-安全计算环境-S3A3G3-现场作业指导书（凝思V6.0操作系统）A3.4'
    ...
    for i, data in enumerate(security_control_class_data, start=0):
        ...
        if llm == 'qwen':
            ...
            if qwen_result != result_score_data[i]:
                ...
                qwenChatResult = qwen_chat(process, result_record_data[i])
                qwen_content, qwen_result = resultDeal(qwenChatResult)
            ...
            if glm4_result != result_score_data[i]:
                glm4ChatResult = glm4_chat(process, result_record_data[i])
                glm4_content, glm4_result = resultDeal(glm4ChatResult)
