...
def glm4_chat(expectResult, result_record, remarks):
    # print(f"expectResult为：{expectResult}")
    if "\n" in expectResult :
        ...
        if remarks is not None:
            attentionContent = f"""
                1.在评估时对具体对象进行比对，识别和理解它们所属的类别或具有的属性一致则为符合。
                2.对预期结果的指令可以不进行描述与判断
                3.当用户通过描述，无需进行某项指标判断时，则该项预期结果为符合。
                4.{remarks}
            """
        else:
            attentionContent = f"""
...
def qwen_chat(expectResult, result_record, remarks):
        ...
        messages = [
            {
                ...
                最后判断，进行说明，如果每条预期结果都符合则输出"结果：符合"，部分预期结果符合输出"结果：部分符合"，每条预期结果都不符合则输出"结果：不符合"。
                """,
            },
            {
                "role": "user",
...
def getExpect(securityCategory , controlPoint, quota, index):
    ...
    for row in sheet.iter_rows(min_col=1, max_col=6, min_row=startIndex, max_row=endIndex, values_only=False):
        cell2 = row[2]  # 第三列的单元格
        exceptResultCell = row[index - 1]  # 第五列的单元格
        processCell = row[index - 2]
        remarksCell = row[index]
        # 检查配额和第五列的值
        if cell2.value is not None and quota in str(cell2.value) and exceptResultCell.value is not None:
            # print(cell5.value)
            return exceptResultCell.value, processCell.value, remarksCell.value
...
if __name__ == "__main__":
    ...
    file_name = 'PCS-9700厂站监控系统2.0.xlsx'
    # file_name = 'RTU交换机1-2.0.xlsx'
    
    instructionFile = '04卓识-安全计算环境-S3A3G3-现场作业指导书（应用系统）A3.4'
    # instructionFile = '04卓识-安全计算环境-S3A3G3-现场作业指导书（华为交换机）A3.4'
...
#     realy_result_col = existing_columns.index("realy_result") + 1
    ...
    for i, data in enumerate(security_control_class_data, start=0):
        ...
        if result_score_data[i] == '不适用':
            continue ;

        # print(result_record_data[i])
        quota = detail_data[i][0:2]
        expectResult, process, remarks = getExpect(instructionFile, security_control_class_data[i], quota, 5)
        print("------------------------------------------------------------------------------------")
        print(f"security_control_class_data[i]为：{security_control_class_data[i]}")
        print(f"quota为：{quota}")
        print(f"预期结果为：{expectResult}")
        print(f"测评过程为：{process}")
        print(f"备注为：{remarks}")
        ...
        # 调用接口
        if llm == 'qwen':
            qwenChatResult = qwen_chat(expectResult, result_record_data[i], remarks)
            qwen_content, qwen_result = resultDeal(qwenChatResult)
            
            if qwen_result != result_score_data[i]:
                
                qwenChatResult = qwen_chat(process, result_record_data[i], remarks)
                qwen_content, qwen_result = resultDeal(qwenChatResult)
            ...
            glm4ChatResult = glm4_chat(expectResult, result_record_data[i], remarks)
            glm4_content, glm4_result = resultDeal(glm4ChatResult)
            
            if glm4_result != result_score_data[i]:
                glm4ChatResult = glm4_chat(process, result_record_data[i], remarks)
                glm4_content, glm4_result = resultDeal(glm4ChatResult)
