...
if __name__ == "__main__":
    ...
    file_name = '后台监控数据库.xlsx'
    
    instructionFile = '04卓识-安全计算环境-S3A3G3-现场作业指导书（MySQL数据库）A3.4'
...
#     realy_result_col = existing_columns.index("realy_result") + 1
    ...
    for i, data in enumerate(security_control_class_data, start=0):
        ...
        expectResult = getExpect(instructionFile, security_control_class_data[i], quota, 5)
        ...
        if llm == 'qwen':
            ...
            if qwen_result != result_score_data[i]:
                operationStep = getExpect(instructionFile, security_control_class_data[i], quota, 4)
        ...
        # 找到正确的行和列，写入'realy_result'和'reason'
